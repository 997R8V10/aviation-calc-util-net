...
namespace AviationCalcUtilNet.Magnetic
{
    ...
    public class MagneticModel
    {
        ...
        public MagneticModel(double epoch, string name, DateTime releaseDate, List<MagneticModelCoefficients> coefficients)
        {
            ...
            ptr = magnetic_magnetic_model_new(epoch, name, InteropUtil.ManagedDateToDateStruct(releaseDate), coeffsArr, new UIntPtr((uint) coeffsArr.Length));
        }
        ...
        [DllImport("aviation_calc_util_ffi", CallingConvention = CallingConvention.Cdecl)] private static extern IntPtr magnetic_magnetic_model_new(double epoch, [MarshalAs(UnmanagedType.LPStr)] string name, InteropDateStruct releaseDate, IntPtr[] coeffs, UIntPtr size);
